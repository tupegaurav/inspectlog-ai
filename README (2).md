...
1. **Frontend** (this repo) — a single-screen form, styled like a factory
   andon/gauge panel, deployed as a static site on GitHub Pages.
2. **Validation + automation** — the form POSTs each entry to an
   [n8n](https://n8n.io) webhook. An AI agent (Groq/LLaMA) checks the entry
...
3. **Record** — valid entries are appended to a Google Sheet automatically;
